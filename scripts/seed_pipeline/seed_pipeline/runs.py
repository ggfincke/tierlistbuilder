...
def run_seed_manifest(
	...
	if _active_release_matches_compiled_manifest(context, state, diff):
		context.progress.log("active release already matches compiled manifest; skipping writes")
...
def _compiled_template_hashes(context: SeedRunContext) -> dict[str, JsonObject]:
	hashes: dict[str, JsonObject] = {
		str(template["externalId"]): {"metadataContentHash": template["metadataContentHash"]}
		for template in cached_template_upserts(context)
	}
	item_rows_by_template = _rows_by_template_external_id(cached_item_upserts(context))
	...
	for template_external_id, rows in criteria_rows_by_template.items():
		...
		hashes.setdefault(template_external_id, {})["criteriaContentHash"] = content_hash
...
def _diff_template_external_ids(section: object, keys: tuple[str, ...]) -> set[str]:
	...
	for key in keys:
		for entry in as_list(section.get(key)):
			if isinstance(entry, dict) and isinstance(entry.get("templateExternalId"), str):
...
def _verify_seed_release(context: SeedRunContext) -> JsonObject:
	...
	for index, chunk in enumerate(template_chunks, start=1):
		...
		diagnostics.extend(
			item for item in as_list(result.get("diagnostics")) if isinstance(item, dict)
		)
...
def _upload_assets(
	...
	total_variants = sum(1 for asset in assets for _variant in asset_variants(asset["asset"]))
	uploaded_variants = 0
	...
	for asset_chunk_index, asset_chunk in enumerate(asset_chunks, start=1):
		...
		variants = [variant for asset in asset_chunk for variant in asset_variants(asset["asset"])]
		context.progress.log(
...
def _generate_upload_urls(context: SeedRunContext, variants: list[JsonObject]) -> list[JsonObject]:
...
def _register_uploaded_storage_ids(context: SeedRunContext, storage_ids: list[str]) -> None:
	batches = list(chunks(storage_ids, CLEANUP_STORAGE_BATCH_SIZE))
	for index, chunk in enumerate(batches, start=1):
...
def _upsert_items(context: SeedRunContext, diff: JsonObject | None = None) -> list[JsonObject]:
	results: list[JsonObject] = []
	batches = child_upsert_batches(cached_item_upserts(context), ITEM_BATCH_SIZE, "items")
...
def _upsert_criteria(context: SeedRunContext, diff: JsonObject | None = None) -> list[JsonObject]:
	results: list[JsonObject] = []
	...
	force_template_external_ids = (
		...
		if diff is not None
		else [str(template["externalId"]) for template in compiled_templates(context.compiled)]
	)
	for index, chunk in enumerate(batches, start=1):
		context.progress.count(
...
def child_upsert_batches(rows: list[JsonObject], limit: int, label: str) -> list[list[JsonObject]]:
...
def _assert_upload_budget(assets: list[JsonObject], max_upload_bytes: int | None) -> None:
