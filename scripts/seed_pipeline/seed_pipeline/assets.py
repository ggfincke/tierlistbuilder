...
def build_variant(
	...
	*,
	...
	output_path = _variant_output_path(source_sha256, kind, variants_dir, variant_spec_version)
...
def compute_variant_dedupe_hash(variants: Iterable[JsonObject]) -> str:
	return "|".join(sorted(f"{variant['kind']}:{variant['contentHash']}" for variant in variants))
