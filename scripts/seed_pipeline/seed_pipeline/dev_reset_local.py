#!/usr/bin/env python3
# scripts/seed_pipeline/seed_pipeline/dev_reset_local.py
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		action="store_true",
		help=(
			"required confirmation flag - without it the script aborts before replacing local state"
		),
	)
	parser.add_argument(
		"--site-url",
...
def active_local_ports() -> list[int]:
	return [port for port in LOCAL_CONVEX_PORTS if is_port_accepting_connections("127.0.0.1", port)]
...
def main() -> int:
	...
	if not args.yes:
		print(
			"\nrefusing to run without --yes. re-run as: npm run db:reset:local-fast -- --yes",
			file=sys.stderr,
		)
		return 2
	...
	try:
		run_checked(["npx", "convex", "dev", "--local", "--once", "--typecheck", "disable"])
		run_checked(
			[
				"node",
