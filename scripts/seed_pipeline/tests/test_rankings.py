...
class RankingSeedCompilationTests(unittest.TestCase):
		...
		templates = [
			{
				...
				"items": [{"externalId": f"item-{index}"} for index in range(1342)],
				"criteria": [
					{
						...
						"curatedRankings": [
							{
								...
								"tierGroups": [{"tierName": "S", "labels": ["Mario"]}],
							}
						],
					}
				],
	...
	def test_apply_retries_convex_write_rate_throttle(self, sleep: object) -> None:
		ranking_seeds = _manifest(
			{
				"profiles": [{"key": "a", "displayName": "A", "chaos": 0, "contrarian": 0}],
				"targets": [
					{
						"templateExternalId": "one",
						"sampleProfileCount": 1,
		...
		self.assertEqual(result["sampleRankingsApplied"] + result["curatedRankingsApplied"], 1)
