...
describe('recolorTierRow', () =>
{
  ...
  it('only affects the target tier', () =>
  {
    ...
    expect(tiers.find((entry) => entry.id === 't-2')?.rowColorSpec).toBeUndefined()
  })
  ...
  it('is undoable & labeled', () =>
  {
    ...
    expect(
      useActiveBoardStore
        .getState()
        .tiers.find((entry) => entry.id === 't-1')?.rowColorSpec
    ).toBeUndefined()
  })
...
describe('normalizeBoardSnapshot row color', () =>
{
  ...
  it('drops invalid rowColorSpec input', () =>
  {
    ...
    const normalized = normalizeBoardSnapshot(raw as Partial<BoardSnapshot>, 'classic')
    expect(normalized.tiers[0].rowColorSpec).toBeUndefined()
  })
})
