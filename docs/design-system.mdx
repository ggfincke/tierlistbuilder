...
graph TD
    ...
    D --> E
    A --> E
    F["Layering policy (local Tailwind z-*)"] --> D
    F --> E
```
...
3. **Layer locally and predictably.** Stacking uses explicit Tailwind `z-*` classes at the owning surface. Reuse the nearest established layer for comparable chrome instead of inventing arbitrary values.
...
Defined per theme in `src/shared/theme/tokens.ts`, written to `:root` by `applyThemeTokens()` in `src/shared/theme/runtime.ts`. There are **26 per-theme color tokens** grouped by role, plus computed/static defaults in `src/app/index.css` for values that are not theme-specific:
...
| Accent      | `--t-accent`, `--t-accent-hover`, `--t-accent-2`, `--t-accent-foreground`, `--t-accent-glow`                            |
| Destructive | `--t-destructive`, `--t-destructive-hover`, `--t-destructive-foreground` (computed contrast)                            |
| Feedback    | `--t-success`, `--t-warning`                                                                                            |
...
Layering is owned by the component that creates the surface. Use the existing local Tailwind layers as precedent: top navigation sits at `z-30`, card menus at `z-20`, skip links and modal-focus affordances at `z-50`, and offscreen export hosts use the `OFFSCREEN_EXPORT_Z_INDEX` constant in `src/features/workspace/export/lib/exportBoardRender.tsx`.

> [!IMPORTANT]
> A new layer is a design decision, not an escape hatch. If an overlay seems to need a one-off arbitrary value, first check whether the surface should instead share an existing modal, popup, card-menu, or top-chrome layer.
...
Used by `ColorPicker`, `TierRowSettingsMenu`, `BoardManager`, `ExportMenu`, card menus, etc.
...
- **Locked themes (embed).** `EmbedRoute` calls `useLockedTheme('scoreboard', 'default')` so embed iframes render the Scoreboard theme regardless of the host's preference. Use this pattern any time you need theme-stable output (export rendering already uses an isolated host).
...
| Layering policy    | Component-local Tailwind `z-*` classes; `OFFSCREEN_EXPORT_Z_INDEX` in `src/features/workspace/export/lib/exportBoardRender.tsx` |
