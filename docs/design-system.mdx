...
graph TD
    ...
    D --> E
    A --> E
    F["Z ladder (shared/theme/zIndex.ts)"] --> D
    F --> E
```
...
3. **One Z rung at a time.** Stacking goes through `Z` in `shared/theme/zIndex.ts` — eight rungs total. Don't invent `z-[27]`.
...
```typescript title="src/shared/theme/zIndex.ts"
export const Z = {
  offscreen: -1, // export hosts that mount but don't render
  base: 0,
  dropdown: 10, // item focus rings, inline canvas text input, drag-raised items
  popup: 20, // selected/active item elevation
  submenu: 30, // primary submenu layer for toolbar popups
  overlay: 40, // floating panels (board manager, bulk action bar)
  modal: 50, // modals, toasts, modal-layer menus
} as const
```

> [!IMPORTANT]
> Adding a new rung is a design decision, not an escape hatch. If you find yourself wanting `submenu + 5`, the right move is usually a different rung or an anchored popup at the same rung — not a new value.
...
| Z ladder           | `src/shared/theme/zIndex.ts`                                                                                          |
