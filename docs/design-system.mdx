...
The keyed modal stack lives in `app/shells/useModalStack.ts` (a feature-shell concern, not `shared/`). Each shell owns its own modal payload map (`app/shells/workspace/workspaceModals.ts`, etc.) and renders modals through `LazyModalSlot` so they don't ship in the initial bundle.
...
- **Locked themes (embed).** `EmbedRoute` calls `useLockedTheme('scoreboard', 'default')` so embed iframes render stable chrome tokens regardless of the host's preference, while the embed view keeps the neutral `classic` tier palette. Use this pattern any time you need theme-stable output (export rendering already uses an isolated host).
