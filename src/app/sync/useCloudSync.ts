// src/app/sync/useCloudSync.ts
// app-level sync coordinator: wires auth epoch, platform transport, & workspace sync
...
import {
  createAppSyncSession,
  type AppSyncSession,
} from './createAppSyncSession'
...
export const useCloudSync = (user: PublicUserMe | null): void =>
{
  ...
  const sessionRef = useRef<AppSyncSession | null>(null)
  ...
  useEffect(() =>
  {
    ...
    const session = createAppSyncSession({
