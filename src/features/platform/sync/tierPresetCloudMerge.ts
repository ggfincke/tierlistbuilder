// src/features/platform/sync/tierPresetCloudMerge.ts
// first-login merge for user-saved tier presets. silent union by externalId,
// w/ pendingOp-driven direction:
//
//   in local store, sidecar pendingOp='upsert'  -> push (overwrites cloud)
//   in local store, no pendingOp                -> no-op (cloud is in sync)
//   not in local store, sidecar pendingOp='delete' & lastSyncedAt set
//                                                -> push delete to cloud
//   in cloud, not in local store                 -> pull (add to local store)
//
// rationale: pendingOp reflects user intent on this device that hasn't
// been flushed yet. without it, "in local store" implies the preset was
// either created here pre-signin (push) or pulled from cloud (don't push
// again). lastSyncedAt distinguishes "I've pushed this preset before"
// from "fresh local creation"
...
// hydrate a cloud row into a TierPreset usable in the store. cloud doesn't
// store the builtIn flag — all cloud-stored presets are user presets by
// definition (built-ins live client-side). cloud externalId is also the
// PresetId since we use the same prefix on both sides
...
): MergeTask[] =>
{
  ...
  for (const preset of localPresets)
  {
    ...
    // synced before & no pending op — cloud is authoritative. if the
    // cloud row is missing, treat that as "another device deleted it"
    // & leave the local copy in place (delete-on-cloud is destructive;
    // we'd rather preserve the user's local copy than silently nuke it
    // on first sign-in)
  }
  ...
  for (const [presetId, row] of cloudIndex)
  {
    ...
    // skip if a sidecar entry says we just deleted it locally — the
    // delete task above will catch up the cloud side, & re-pulling
    // would resurrect the row in the local store between the delete
    // task running & the cloud query refreshing
