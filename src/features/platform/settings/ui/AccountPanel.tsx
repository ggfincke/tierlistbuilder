...
const AccountSection = ({ user }: { user: PublicUserMe }) =>
{
  ...
  return (
    <SetSection eyebrow="Sign in" title="Account">
      ...
      {/*
        TODO(account-api): 2FA + recovery codes. Password change is wired
        through account actions, but TOTP/recovery codes are not.
      */}
...
const PlanSection = ({ plan }: { plan: UserPlan }) => (
  <SetSection eyebrow="Billing" title="Plan & billing">
    ...
    {/*
      TODO(billing): Stripe billing. `plan` is real, but there is no billing
