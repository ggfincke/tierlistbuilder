...
- `ui/AccountModalShell.tsx` — account-management shell for profile/session/danger-zone chrome. Product-specific account sections are injected by `app/shells/topNav/AccountModal.tsx` so platform auth does not import marketplace UI.
...
- Platform auth UI must not import marketplace/library/workspace product sections directly — compose those at the app shell.
