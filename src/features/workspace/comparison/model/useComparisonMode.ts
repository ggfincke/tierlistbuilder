// src/features/workspace/comparison/model/useComparisonMode.ts
...
import { loadPersistedBoard } from '@/features/workspace/boards/data/local/localBoardSession'
import { useWorkspaceBoardRegistryStore } from '@/features/workspace/boards/model/useWorkspaceBoardRegistryStore'
import { useActiveBoardStore } from '@/features/workspace/boards/model/useActiveBoardStore'

// load board data either from the active store or from storage
const loadBoard = (boardId: BoardId): BoardSnapshot =>
{
  const { activeBoardId } = useWorkspaceBoardRegistryStore.getState()
  if (boardId === activeBoardId)
  {
    return extractBoardData(useActiveBoardStore.getState())
  }
