...
export const stripImagesForShare = (data: TierListData): TierListData =>
{
  const strippedItems: TierListData['items'] = {}
...
// app base URL (origin + pathname, no hash or query)
export const getAppBaseUrl = (): string =>
  `${window.location.origin}${window.location.pathname}`

// build the full shareable URL w/ hash fragment
export const getShareUrl = async (data: TierListData): Promise<string> =>
{
  const fragment = await encodeBoardToShareFragment(data)
  return `${getAppBaseUrl()}#share=${fragment}`
}

// rough byte estimate of the encoded share URL
export const estimateShareSize = (data: TierListData): number =>
{
  const stripped = stripImagesForShare(data)
  const json = JSON.stringify(stripped)
